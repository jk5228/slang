...
rules = defaultdict(list, {'retStm':[['return'],['return','exp']],'arrComp':[['{','id','in','exp',':','exp','}'],['{','id','in','exp',':','exp','->','exp','}']],'exp':[['(','exp',')'],['prim'],['assign'],['arrAcc'],['funExp'],['arrExp'],['rngExp'],['arrComp'],['arithExp'],['logExp']],'funBlk':[['def','id','(','idLst',')','{','stmLst','}']],'aOp2':[['+'],['-'],['*'],['/'],['%']],'arithExp':[['exp','aOp2','exp']],'exp+':[['exp'],['exp',',','exp+']],'id+':[['id'],['id',',','id+']],'lOp1':[['!']],'stm+':[['stm'],['stm','stm*']],'exp*':[[],['exp+']],'prim':[['num'],['str'],['id']],'block':[['funBlk'],['ifBlk'],['whileBlk'],['forBlk']],'arrExp':[['{','expLst','}']],'whileBlk':[['while','(','exp',')','{','stmLst','}']],'expLst':[['exp*']],'forBlk':[['for','(','id','in','exp',')','{','stmLst','}']],'lOp2':[['&&'],['||'],['=='],['<='],['>='],['>'],['<']],'line':[['exp'],['break'],['retStm']],'stm':[['line',';'],['block']],'id*':[[],['id+']],'ifBlk':[['if','(','exp',')','{','stmLst','}','else','{','stmLst','}']],'rngExp':[['exp','..','exp'],['exp','...','exp']],'logExp':[['lOp1','exp'],['exp','lOp2','exp']],'arrAcc':[['id','[','exp',']']],'idLst':[['id*']],'stmLst':[['stm*']],'stm*':[[],['stm+']],'assign':[['id','=','exp'],['arrAcc','=','exp']],'funExp':[['id','(','expLst',')']],'prog':[['stm*']]})
