...
rules = defaultdict(list, {'exp*':[[],['exp+']],'logExp':[['lOp1','exp'],['exp','lOp2','exp']],'lOp1':[['!']],'lOp2':[['&&'],['||'],['=='],['<='],['>='],['>'],['<']],'aOp2':[['+'],['-'],['*'],['/'],['%']],'stm*':[[],['stm+']],'arithExp':[['exp','aOp2','exp']],'whileBlk':[['while','(','exp',')','{','stmLst','}']],'funExp':[['id','(','expLst',')']],'funBlk':[['def','id','(','idLst',')','{','stmLst','}']],'line':[['exp'],['break'],['retStm']],'stm':[['line',';'],['block']],'arrExp':[['{','expLst','}']],'exp':[['(','exp',')'],['prim'],['assign'],['arrAcc'],['funExp'],['arrExp'],['arrComp'],['arithExp'],['logExp']],'stmLst':[['stm*']],'forBlk':[['for','(','id','in','exp',')','{','stmLst','}']],'block':[['funBlk'],['ifBlk'],['whileBlk'],['forBlk']],'retStm':[['return'],['return','exp']],'prim':[['num'],['str'],['id']],'arrAcc':[['id','[','exp',']']],'assign':[['id','=','exp'],['arrAcc','=','exp']],'id*':[[],['id+']],'stm+':[['stm'],['stm','stm*']],'expLst':[['exp*']],'ifBlk':[['if','(','exp',')','{','stmLst','}','else','{','stmLst','}']],'arrComp':[['{','id','in','exp',':','exp','}'],['{','id','in','exp',':','exp','->','exp','}']],'idLst':[['id*']],'id+':[['id'],['id',',','id+']],'prog':[['stm*']],'exp+':[['exp'],['exp',',','exp+']]})
