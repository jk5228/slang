...
rules = defaultdict(list, {'ifBlk':[['if','(','exp',')','{','stmLst','}','else','{','stmLst','}']],'lOp1':[['!']],'line':[['exp'],['assign'],['break'],['retStm']],'logExp':[['lOp1','exp'],['(','exp',')','l2op','exp']],'forBlk':[['for','(','id','in','exp',')','{','stmLst','}']],'prog':[['stm*']],'funExp':[['id','(','expLst',')']],'stm+':[['stm'],['stm','stm*']],'exp':[['(','exp',')'],['prim'],['arrAcc'],['funExp'],['arrExp'],['arithExp'],['logExp']],'block':[['funBlk'],['ifBlk'],['whileBlk'],['forBlk']],'lOp2':[['&&'],['||'],['=='],['>'],['<']],'stm':[['line',';'],['block']],'funBlk':[['def','id','(','idLst',')','{','stmLst','}']],'id*':[[],['id+']],'exp*':[[],['exp+']],'retStm':[['return'],['return','exp']],'whileBlk':[['while','(','exp',')','{','stmLst','}']],'arithExp':[['(','exp',')','aOp2','exp']],'assign':[['id','=','exp'],['arrAcc','=','exp']],'prim':[['num'],['str'],['id']],'exp+':[['exp'],['exp',',','exp+']],'expLst':[['exp*']],'stm*':[[],['stm+']],'arrExp':[['{','expLst','}']],'id+':[['id'],['id',',','id+']],'idLst':[['id*']],'aOp2':[['+'],['-'],['*'],['/']],'stmLst':[['stm*']],'arrAcc':[['id','[','exp',']']]})
